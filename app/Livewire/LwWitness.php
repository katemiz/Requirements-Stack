...
class LwWitness extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        $this->constants = config('witnesses');
    }
    ...
    public function render()
    {
        ...
        return view('projects.witnesses.lw-witnesses',[
            'witnesses' => $this->getWitnessesList()
        ]);
    }
    ...
    {
        Witness::find($this->uid)->delete();
        session()->flash('message','Project witness definition has been deleted successfully.');
        $this->action = 'LIST';
        $this->resetPage();
    }
    ...
    public function storeUpdateItem () {
        ...
        if ( $this->uid ) {
            // update
            Witness::find($this->uid)->update($props);
            session()->flash('message','Project witness definition has been updated successfully.');

        } else {
            // create
            $props['user_id'] = Auth::id();
            $this->uid = Witness::create($props)->id;
            session()->flash('message','Project witness definition has been created successfully.');
        }
